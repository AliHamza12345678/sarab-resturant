...
function openMenuPop(card) {
    ...
    menuPop.setAttribute('data-active-id', id);
...
document.getElementById('mpAddCart').addEventListener('click', function() {
    var id = parseInt(menuPop.getAttribute('data-active-id'));
    var title = document.getElementById('mpTitle').textContent;
    var priceText = document.getElementById('mpPrice').textContent.replace('$', '').trim();
    var price = parseFloat(priceText);
    var img = document.getElementById('mpImg').getAttribute('src');
    
    if (isNaN(id) || !title) return;

    var cart = JSON.parse(localStorage.getItem('restaurant_cart')) || [];
    var existingItem = cart.find(item => item.id === id);
    if (existingItem) {
        existingItem.quantity += mpQty;
    } else {
        cart.push({
            id: id,
            title: title,
...
// Contact form now submits for real to contact.php (server-side handling).
// Just show a "Sending..." state on click; the real result comes back
// from the server after the page reloads.
var ctcBtnEl = document.getElementById('ctcBtn');
if (ctcBtnEl) {
    ctcBtnEl.addEventListener('click', function() {
        var btn = this;
        setTimeout(function() {
...
var ctcOkEl = document.getElementById('ctcOk');
if (ctcOkEl && ctcOkEl.style.display === 'block') {
    ctcOkEl.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
